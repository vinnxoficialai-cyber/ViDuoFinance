      ...
      <div className="flex justify-between items-start mb-6">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            {account.closingDay ? (
              <span className="text-[9px] text-amber-400 font-bold flex items-center gap-1 uppercase tracking-tighter">
                <Calendar size={10} /> Vence dia {account.closingDay}
              </span>
            ) : (
...
const Contas: React.FC = () => {
  ...
  const [newAccount, setNewAccount] = useState<any>({
    ...
    owner: 'Conjunta',
    lastDigits: '',
    closingDay: '10' // Valor padrão para o vencimento
  });
  ...
  const handleAddAccount = (e: React.FormEvent) => {
    ...
    addAccount({
      ...
      closingDay: Number(newAccount.closingDay) || 10,
      trend: 0
    });

    setShowAddForm(false);
    setNewAccount({ name: '', balance: 0, color: 'purple', owner: 'Conjunta', lastDigits: '', closingDay: '10' });
  };
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h2 className="text-3xl font-black tracking-tight flex items-center gap-3">
             ...
             Carteira Digital Duo
          </h2>
          <p className="text-zinc-500 text-sm font-medium mt-1">Gestão centralizada de ativos e cartões do casal.</p>
        </div>
          ...
          <div className="bg-white dark:bg-zinc-900 w-full max-w-lg rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in-95 duration-300 relative border border-zinc-200 dark:border-zinc-800">
             <div className="flex justify-between items-center mb-8">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-xl font-black tracking-tighter uppercase">Nova Conta / Banco</h3>
                </div>
             ...
             <form onSubmit={handleAddAccount} className="space-y-6">
                <div className="space-y-1.5">
                  ...
                  <input required placeholder="Ex: NuBank, Inter, Itaú" className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-4 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-purple-600/20" value={newAccount.name} onChange={e => setNewAccount({...newAccount, name: e.target.value})} />
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black uppercase text-zinc-400 tracking-widest ml-1">Saldo Inicial (R$)</label>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black uppercase text-zinc-400 tracking-widest ml-1">Vencimento Fatura</label>
                    <input type="number" placeholder="Dia (Ex: 10)" className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-4 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-purple-600/20" value={newAccount.closingDay} onChange={e => setNewAccount({...newAccount, closingDay: e.target.value})} />
                  </div>
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
         <div className="bg-white dark:bg-zinc-900/50 p-6 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm flex items-center gap-4">
            ...
            <div>
               <p className="text-[10px] font-black uppercase text-zinc-400 tracking-widest">Crescimento Duo</p>
               <h4 className="text-lg font-black">+4.2% esta semana</h4>
            </div>
         </div>
         <div className="bg-white dark:bg-zinc-900/50 p-6 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm flex items-center gap-4">
            ...
            <div>
               <p className="text-[10px] font-black uppercase text-zinc-400 tracking-widest">Distribuição</p>
               <h4 className="text-lg font-black">{accounts.filter(a => a.owner === 'Conjunta').length} Contas em Parceria</h4>
            </div>
         </div>
         <div className="bg-white dark:bg-zinc-900/50 p-6 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm flex items-center gap-4">
            ...
            <div>
               <p className="text-[10px] font-black uppercase text-zinc-400 tracking-widest">Segurança DuoFinance</p>
               <h4 className="text-lg font-black">Dados Protegidos</h4>
            </div>
         </div>
      </div>
    </div>
